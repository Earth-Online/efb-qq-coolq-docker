...
ENV PROFILE default
RUN apk --no-cache --virtual build add build-base libffi-dev openssl-dev && apk --no-cache add jpeg-dev zlib-dev libwebp-dev libmagic ffmpeg && pip install -r requirements.txt && rm -rf ~/.cache && apk del build
